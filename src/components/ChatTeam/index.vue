    ...
    export default {
        ...
        watch: {
            ...
            _groupId(newValue,oldValue){
                let vm = this
                vm.$socket.removeAllListeners('chanel_group_'+oldValue)
                vm.$socket.on('chanel_group_'+newValue, function (data) {
                    console.log(data)
                })
            }
        },
        ...
        methods: {
            ...
            getMessages(){
                let idGroup = this.$router.params.id
                axios.get(`http://localhost:3000/groups/${idGroup}/messages`).then(data => {
